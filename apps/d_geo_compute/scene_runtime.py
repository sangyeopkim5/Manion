    ...
    def construct(self):
        ...
        for ex in spec.get("extras", []):
            ...
            if ex["type"] == "len_label":
                p, q = pts[ex["from"]], pts[ex["to"]]
                text = MathTex(ex["text"]).scale(0.8).move_to(
                    (p + q) / 2 + np.array(ex.get("offset", [0, 0, 0]))
                )
            ...
            if ex["type"] == "angle":
                ...
                P = pts[ex["p"]]
                Q = pts[ex["q"]]
                l1, l2, arc = make_angle_inner(
                    V, P, Q, radius=ex.get("radius", 0.55), color=YELLOW
                )
                given = float(ex["deg"])
                meas = measured_inner_deg(V, P, Q)
                lbl = label_dual(V, arc, given, meas, scale=0.9)
                self.play(Create(l1), Create(l2), Create(arc), Write(lbl))

        def L(t, p, where):
            self.add(Text(t, font_size=28).next_to(Dot(p), where, buff=0.12))
