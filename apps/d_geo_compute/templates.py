...
def solve_quad_diaglen_ang(A, D, theta_A_deg, theta_D_deg, AC_len, BD_len):
    ...
    uAD = unit(D - A)
    uDA = -uAD
    thA = np.deg2rad(theta_A_deg)
    thD = np.deg2rad(theta_D_deg)
    for sA in (+1, -1):
        for sD in (+1, -1):
            C = A + rotate(uAD, sA * thA) * AC_len
            B = D + rotate(uDA, sD * thD) * BD_len
            if seg_x(A, B, C, D) or seg_x(B, C, A, D):
                continue
            if area_signed([A, B, C, D]) <= 0:
                continue
            if not diag_cross_inside(A, B, C, D):
                continue
