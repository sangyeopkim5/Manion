...
def v3(x, y, z=0.0): return np.array([float(x), float(y), float(z)], dtype=float)
...
def seg_x(p1,p2,p3,p4):
    o1=orient2d(p1,p2,p3); o2=orient2d(p1,p2,p4); o3=orient2d(p3,p4,p1); o4=orient2d(p3,p4,p2)
...
def area_signed(poly):
    P=np.array(poly, float); x=P[:,0]; y=P[:,1]
    return 0.5*float(np.dot(x, np.roll(y,-1)) - np.dot(y, np.roll(x,-1)))

def ensure_ccw(*pts):
    return pts if area_signed(pts)>0 else (pts[0], pts[3], pts[2], pts[1])

def diag_cross_inside(A,B,C,D): return seg_x(A,C,B,D)

def fit_into_box(pts, box_min, box_max, margin=0.2):
    P = np.array(pts, float)
    pmin = P[:,:2].min(axis=0); pmax = P[:2].max(axis=0) if P.ndim==1 else P[:,:2].max(axis=0)
    pmax = P[:,:2].max(axis=0)
    size = pmax - pmin
